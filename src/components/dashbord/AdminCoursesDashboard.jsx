...
//import CourseCreationForm from "./CourseCreationForm"; // Import the CourseCreationForm component

const AdminCoursesDashboard = () => {
  const [courses, setCourses] = useState([]);
  const [filter, setFilter] = useState("");
  const [isAddModalOpen, setAddModalOpen] = useState(false); // Manage modal visibility
          ...
          {filteredCourses.map((course) => (
            <tr key={course._id}>
              ...
              <td className="border p-2 flex gap-2">
                <button
                  onClick={() => setSelectedCourse(course)} // Edit course
